...
fn date_time_range() -> (String, String) {
    ...
    (yesterday.format("%Y-%m-%d").to_string(), now.format("%Y-%m-%d").to_string())
}

fn get_horizons_data(body_name: String) -> io::Result<(String)> {
    let body_id = HORIZONS_IDS[body_name.as_str()];

    let horizons_data_file = format!("target/{}_data.json", body_name);
    let use_cached_file = if Path::new(horizons_data_file.as_str()).exists() {
        let user_choice = !take_user_choice("Used cached file?");
        user_choice
    } else {
